...
if(DB_DRIVER == DB_MYSQL)
{
	...
	{
		$db = new Mysql;
		$result = $db->select()->setColumns('t.key, t.data')->setTable(MYSQL_TABLE_NAME)->setWhere("`key` = 'user9'")->exec();
	}
...
if(DB_DRIVER == DB_POSTGRE)
	...
	{
		$db = new Postgresql;
		$result = $db->select()->setColumns('t.key, t.data')->setTable(POSTGRE_TABLE_NAME)->setWhere("`key` = 'user9'")->exec();
		var_dump($result);
	}
	...
	if(isset($_POST['addRow']) && !empty($_POST['string']))
	{
		...
		$params = array('key' => 'user9', 'data' => "$string");
		$db = new Postgresql;
		$result = $db->insert()->setTable(POSTGRE_TABLE_NAME)->setParams($params)->exec();
